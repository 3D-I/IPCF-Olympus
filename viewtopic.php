...
$sql = 'SELECT p.post_id, p.poster_ip
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	if (($row['post_edit_count'] && $config['display_last_edited']) || $row['post_edit_reason'])
	{
		...
		if (!sizeof($post_edit_list) && ($row['post_edit_reason'] || ($row['post_edit_user'] && !isset($user_cache[$row['post_edit_user']]))))
		{
			...
			// --- IP Country Flag Olympus start --------------------
			// add
			// p.poster_ip, u.user_ip,
			$sql = 'SELECT DISTINCT p.poster_ip, u.user_id, u.username, u.user_colour, u.user_ip
...
// --- IP Country Flag Olympus start --------------------
// IP on registration
				$flag = ip_country_to_flag(userid_to_userip($row['post_edit_user']));
...
*/
				$display_username = get_username_string('full', $poster_id, '<img src="images/flags/small/' .  $flag . '.png" width="14" height="9" alt="' . $user->lang['country'][$country] . '" title="' . $user->lang['country'][$country] . '" />' . ' ' . $row['username'], $row['user_colour'], $row['post_username']);
// --- IP Country Flag Olympus end ----------------------
			}
			else
			{
...
*/
				$display_username = get_username_string('full', $row['post_edit_user'], '<img src="images/flags/small/' .  $flag . '.png" width="14" height="9" alt="' . $user->lang['country'][$country] . '" title="' . $user->lang['country'][$country] . '" />' . ' ' . $post_edit_list[$row['post_edit_user']]['username'], $post_edit_list[$row['post_edit_user']]['user_colour']);
// --- IP Country Flag Olympus end ----------------------
			}
...
// --- IP Country Flag Olympus start --------------------
// IP for this post
	$flag = ip_country_to_flag($row['poster_ip']);
	$country = strtoupper($flag);
// --- IP Country Flag Olympus end ----------------------
	//
	$postrow = array(
...
*/
		'POST_AUTHOR_FULL'		=> get_username_string('full', $poster_id, '<img src="images/flags/small/' .  $flag . '.png" width="14" height="9" alt="' . $user->lang['country'][$country] . '" title="' . $user->lang['country'][$country] . '" />' . ' ' . $row['username'], $row['user_colour'], $row['post_username']),
// --- IP Country Flag Olympus end ----------------------
