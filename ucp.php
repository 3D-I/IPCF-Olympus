...
// add , u.user_ip
	$sql = $db->sql_build_query('SELECT_DISTINCT', array(
		'SELECT'	=> 'u.user_id, u.username, u.user_ip, u.username_clean, u.user_colour, MAX(s.session_time) as online_time, MIN(s.session_viewonline) AS viewonline',
...
*/
			'USERNAME_FULL'	=> get_username_string('full', $row['user_id'], '<img src="images/flags/small/' .  $flag . '.png" width="14" height="9" alt="' . $user->lang['country'][$country] . '" title="' . $user->lang['country'][$country] . '" />' . ' ' . $row['username'], $row['user_colour']))
// --- IP Country Flag Olympus end ----------------------
		);
	}
	$db->sql_freeresult($result);
