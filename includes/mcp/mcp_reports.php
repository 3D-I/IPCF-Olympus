...
// add
// , u.user_ip
				$sql = 'SELECT r.post_id, r.user_id, r.report_id, r.report_closed, report_time, r.report_text, rr.reason_title, rr.reason_description, u.username, u.username_clean, u.user_colour, u.user_ip
...
// --- IP Country Flag Olympus start --------------------
// for post - IP on session
				$flag = ip_country_to_flag($post_info['poster_ip']);
				$country = strtoupper($flag);
// for reporter - IP on registration
				$flag_reporteer = ip_country_to_flag($report['user_ip']);
				$country_reporteer = strtoupper($flag_reporteer);
// --- IP Country Flag Olympus end ----------------------
				$template->assign_vars(array(
					'S_MCP_REPORT'			=> true,
...
*/
					'POST_AUTHOR_FULL'		=> get_username_string('full', $post_info['user_id'], '<img src="images/flags/small/' .  $flag . '.png" width="14" height="9" alt="' . $user->lang['country'][$country] . '" title="' . $user->lang['country'][$country] . '" />' . ' ' . $post_info['username'], $post_info['user_colour'], $post_info['post_username']),
// --- IP Country Flag Olympus end ----------------------
...
*/
					'REPORTER_FULL'				=> get_username_string('full', $report['user_id'], '<img src="images/flags/small/' .  $flag_reporteer . '.png" width="14" height="9" alt="' . $user->lang['country'][$country_reporteer] . '" title="' . $user->lang['country'][$country_reporteer] . '" />' . ' ' . $report['username'], $report['user_colour']),
// --- IP Country Flag Olympus end ----------------------
...
// , p.poster_ip
				...
				{
					$sql = 'SELECT t.forum_id, t.topic_id, t.topic_title, p.post_id, p.post_subject, p.post_username, p.poster_id, p.post_time, p.poster_ip, u.username, u.username_clean, u.user_colour, r.user_id as reporter_id, ru.username as reporter_name, ru.user_colour as reporter_colour, r.report_time, r.report_id
...
// --- IP Country Flag Olympus start --------------------
// for post - IP on session
						$flag_post_report_closed = ip_country_to_flag($row['poster_ip']);
						$country_post_report_closed = strtoupper($flag_post_report_closed);
// for reporter - IP on registration
						$flag_report_closed = ip_country_to_flag(userid_to_userip($row['reporter_id']));
						$country_report_closed = strtoupper($flag_report_closed);
// --- IP Country Flag Olympus end ----------------------
...
*/
							'POST_AUTHOR_FULL'		=> get_username_string('full', $row['poster_id'], '<img src="images/flags/small/' .  $flag_post_report_closed . '.png" width="14" height="9" alt="' . $user->lang['country'][$country_post_report_closed] . '" title="' . $user->lang['country'][$country_post_report_closed] . '" />' . ' ' . $row['username'], $row['user_colour'], $row['post_username']),
// --- IP Country Flag Olympus end ----------------------
...
// replace
/*
							'REPORTER_FULL'			=> get_username_string('full', $row['reporter_id'], $row['reporter_name'], $row['reporter_colour']),me']),
*/
							'REPORTER_FULL'			=> get_username_string('full', $row['reporter_id'], '<img src="images/flags/small/' .  $flag_report_closed . '.png" width="14" height="9" alt="' . $user->lang['country'][$country_report_closed] . '" title="' . $user->lang['country'][$country_report_closed] . '" />' . ' ' . $row['reporter_name'], $row['reporter_colour']),
// --- IP Country Flag Olympus end ----------------------
